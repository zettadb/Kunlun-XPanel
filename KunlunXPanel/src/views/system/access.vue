<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <div v-if="user_grant==='Y'?true:false" class="table-list-wrap">
        <el-button
          class="filter-item"
          type="primary"
          icon="el-icon-plus"
    ...
    >
      ...
      >
        <template slot-scope="{row,$index}">
          <el-button
            v-show="user_grant==='Y'&&row.username!=='super_dba'"
            type="primary"
            size="mini"
      ...
      >
        ...
        <div v-if="temp.radio=='2'">
          ...
          >
            ...
            <el-checkbox v-for="cluster in clusters" :key="cluster.id" :label="cluster.id">{{ cluster.nick_name }}
            </el-checkbox>
          </el-checkbox-group>
        </div>
        ...
        if (valid) {
          ...
          if (this.temp.valid_period == 'from_to') {
            if (!this.temp.start_ts && !this.temp.end_ts) {
              messageTip(`起止时间起止时间必须有一个不为空`, 'info')
              return
            }
            if (this.temp.start_ts && this.temp.end_ts) {
              if (this.temp.start_ts >= this.temp.end_ts) {
                messageTip(`开始时间不能大于等于结束时间`, 'info')
                return
              }
            }
          } else {
