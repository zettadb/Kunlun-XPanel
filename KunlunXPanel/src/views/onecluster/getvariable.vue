<template>
  <div class="app-container">
    <el-form ref="form" :model="form" label-width="100px" :rules="rules">
      ...
      <el-form-item label="获取变量名:" prop="variable">
        <el-input
          v-model="form.variable"
          placeholder="show variables like 'fullsync_timeout'"
        />
      </el-form-item>
      <el-form-item>
    ...
    onSubmit(row) {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          const paras = {}
          const ip_arr = row.replica.substring(0, row.replica.length - 1).split('(')
          ...
          getVariable(tempData).then((response) => {
            ...
            if (res.error_code === 0) {
              this.message_tips = '获取实例变量成功,value=' + res.list[0].Value
              this.message_type = 'success'
