...
export default {
  ...
  methods:{
        ...
        if (valid) {
          ...
          addComps(tempData).then((response) => {
            ...
            if(res.status=='accept'){
              ...
              //把之前的数据清空
              this.computer=[];this.computer_state='';this.computer_title='';this.computer_icon='';this.comp_active=0;this.init_title='';this.init_active=0;this.finish_title='';this.finish_icon='';this.finish_description='';this.computer_description='';this.job_id='';this.timer=null;
    ...
    handleDelete(row) {
      ...
      getCompsCount(temp).then((res) => {
        if(res.total==1){
          ...
          let string="此操作将永久删除计算节点"+row.hostaddr+"("+row.port+"), 是否继续?code="+code;
    ...
    handleControlNode(row,action) {
      ...
      const code=createCode();
      let string="将对该计算节点"+row.hostaddr+"("+row.port+")进行"+action_name+"操作,是否继续?code="+code;
      gotoCofirm(string).then( (res) =>{  
        //先执行删权限
        if(!res.value){
          this.message_tips = 'code不能为空！';
