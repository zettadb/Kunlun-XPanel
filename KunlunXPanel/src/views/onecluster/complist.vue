  ...
  <div class="app-container">
    <div class="filter-container">
      <div class="table-list-search-wrap">
        ...
        <el-button v-if="compute_node_create_priv === 'Y'" class="filter-item" type="primary" icon="el-icon-plus"
          @click="handleCreate">添加计算节点
        </el-button>
      </div>
    </div>
    <el-table ref="multipleTable" :key="tableKey" v-loading="listLoading" :data="list" border highlight-current-row
      ...
      <el-table-column prop="name" align="center" label="计算节点名称" />
      <el-table-column prop="status" align="center" label="状态" width="170">
      ...
      <el-table-column label="操作" align="center" width="400" class-name="small-padding fixed-width">
        <template slot-scope="{ row, $index }">
          ...
          <el-button v-if="row.status !== 'manual_stop'" size="mini" type="primary"
            @click="handleControlNode(row, 'stop')">禁用
          </el-button>
          <el-button size="mini" type="primary" @click="handleControlNode(row, 'restart')">重启</el-button>
          <el-button v-if="compute_node_drop_priv === 'Y'" size="mini" type="danger"
      ...
      <el-form ref="dataForm" :model="temp" :rules="rules" label-position="left" label-width="130px">
        ...
        <el-form-item label="选择计算机:" prop="machinelist">
          <el-select v-model="temp.machinelist" multiple placeholder="请选择">
            <el-option v-for="machine in compmachines" :key="machine.id" :label="machine.hostaddr"
