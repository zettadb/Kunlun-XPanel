    ...
    onSubmit(row) {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          const string = '将对' + this.form.shard_name + '进行主备切换,是否继续?code=' + code
          gotoCofirm(string).then((res) => {
            ...
            if (!res.value) {
              ...
              const master_ip = (this.form.primary_node.substring(0, this.form.primary_node.length - 1)).replace('(', '_')
              const paras = {}
              if (!this.form.replica.length) {
                ...
                const assign_ip = (this.form.replica.substring(0, this.form.replica.length - 1)).replace('(', '_')
                paras.shard_id = this.form.shard_id
                paras.cluster_id = this.form.id
                paras.master_hostaddr = master_ip
      ...
      setTimeout(() => {
        ...
        getEvStatus(postarr).then((res) => {
          if (info == '主备切换') {
            ...
            } else if (res.error_code === '70001' && res.status === 'failed') {
              if (i > 5) {
                clearInterval(timer)
              }
            }
