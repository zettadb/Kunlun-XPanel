<template>
  <div class="app-container">
    ...
    <div style="float:right;width:80%;height:630px;">
      ...
      <el-button type="text" size="mini"  style="padding-left:30px;font-size:14px;" @click="handleResult()" v-show="flag" class="el-icon-video-play">运行</el-button>
      <el-button type="text" size="mini"  style="padding-left:10px;font-size:14px;" @click="sqlFormat()"  class="el-icon-magic-stick">格式化SQL</el-button>
      <el-button type="text" size="mini"  style="padding-left:10px;font-size:14px;" @click="del()"  class="el-icon-delete">一键清空</el-button>
      ...
      <textarea style="border:0;
            ...
            placeholder="请输入sql语句..." maxlength="2000"  v-model="stmt" @input="descInput" class="codesql"  ref="mycode" ></textarea>
            <!-- class="codesql"  ref="mycode" -->
            <!-- <span class="numLimit">{{remnant}}/2000</span> -->
      <div style="width:100%;height:70%;margin-top:15px;">
       
        <el-tabs v-model="activeName" type="card">
          <el-tab-pane label="信息" name="1" v-html="info">{{info}}</el-tab-pane>
...
export default {
  ...
  mounted(){
    ...
    this.editor = CodeMirror.fromTextArea(this.$refs.mycode, {
      //mode: { name: 'javascript', json: true }, // 选择对应代码编辑器的语言，我这边选的是数据库，根据个人情况自行设置即可
      mode:mime,
      indentWithTabs: true,
      smartIndent: true,
      lineNumbers: true,
  ...
  methods: {
    ...
    descInput(){
      console.log(11);
      var txtVal = this.editor.getValue().length;
    ...
    handleResult(){
      ...
      if(this.editor.getValue().length>2000){
        this.flag=true;
    ...
    sqlFormat() {
      ...
      /* 将sql内容进⾏格式后放⼊编辑器中*/
       this.editor.setValue(sqlFormatter.format("select * from test;"))
      //console.log(sqlFormatter.format(sqlContent));return;
    },
    del(){
