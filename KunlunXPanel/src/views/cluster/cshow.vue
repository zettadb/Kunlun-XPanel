<template>
  <div>
    <el-radio-group v-model="currentCase" size="small" @change="agreeChange" v-show="g_loading">
      <el-radio v-for="cluster in clusters" :label="cluster.id"
        :key="cluster.id">{{ cluster.nick_name + '(' + cluster.id + ')' }}</el-radio>
    </el-radio-group>
    <div class="nodata" v-show="nodataShow">暂无数据</div>
    <!-- style="margin-top:0px;width: calc(100(100% - 10px);height:calc(100vh - 160px);" -->
    <div ref="myPage" style="margin-top:0px;width: calc(100(100% - 10px);height:calc(100vh - 160px);" v-show="g_loading"
    ...
    <div v-show="isShowNodeMenuPanel && g_loading"
      ...
      <div class="c-node-menu-item" v-if="type === 'cluster' && (shard_create_priv === 'Y')" @click.stop="doAction('新增存储集群')">
        新增存储集群</div>
      <div class="c-node-menu-item" v-if="type === 'cluster' && (compute_node_create_priv === 'Y')"
        @click.stop="doAction('新增计算节点')">新增计算节点</div>
      ...
      <div class="c-node-menu-item" v-if="(type === 'cnode' || type === 'snode') && (node_status == 'active')"
        @click.stop="doAction('节点监控')">节点监控</div>
    </div>
    ...
    <el-dialog title="新增存储集群" :visible.sync="dialogShardVisible" custom-class="single_dal_view">
      <el-form ref="restoreForm" :model="shardtemp" :rules="rules" label-position="left" label-width="110px">
        <el-form-item label="选择计算机:" prop="machinelist">
          <el-checkbox-group v-model="shardtemp.machinelist" :min="minMachine" :max="machineTotal">
            <el-checkbox v-for="machine in machines" :label="machine.hostaddr"
              :key="machine.id">{{ machine.hostaddr }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        ...
        <el-form-item label="副本数:" prop="snode_count">
          <el-input v-model="shardtemp.snode_count" placeholder="副本数至少是3，且不能大于所选计算机数"
    ...
    <el-dialog title="新增计算节点" :visible.sync="dialogCompVisible" custom-class="single_dal_view">
      <el-form ref="compForm" :model="comptemp" :rules="rules" label-position="left" label-width="110px">
        <el-form-item label="选择计算机:" prop="machinelist">
          <el-checkbox-group v-model="comptemp.machinelist" :min="minMachine" :max="machineTotal">
            <el-checkbox v-for="machine in machines" :label="machine.hostaddr"
              :key="machine.id">{{ machine.hostaddr }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
    ...
    <el-dialog title="新增存储节点" :visible.sync="dialogSnodeVisible" custom-class="single_dal_view">
      <el-form ref="compForm" :model="comptemp" :rules="rules" label-position="left" label-width="110px">
        <el-form-item label="选择计算机:" prop="machinelist">
          <el-checkbox-group v-model="snodetemp.machinelist" :min="minMachine" :max="machineTotal">
            <el-checkbox v-for="machine in machines" :label="machine.hostaddr"
              :key="machine.id">{{ machine.hostaddr }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
    ...
    <el-dialog title="设置延迟告警时间" :visible.sync="dialogDalayVisible" custom-class="single_dal_view">
      <el-form ref="dalayForm" :model="dalaytemp" :rules="rules" label-position="left" label-width="120px">
        <el-form-item label="shard名称:" prop="shard_name">
    ...
    <el-dialog title="存储节点详情" :visible.sync="dialogDetailVisible" custom-class="single_dal_view">
      <div class="box">
        <div title="">
          <el-form ref="detailForm" :model="detailtemp" :rules="rules" label-position="left" label-width="120px">
        ...
        <div title="">
          <el-form ref="detailForm" :model="detailtemp" :rules="rules" label-position="left" label-width="150px">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="variableVisible" custom-class="single_dal_view">
      <el-form ref="variableForm" :model="variabletemp" :rules="rules" label-position="left" label-width="120px">
        <el-form-item label="IP:" prop="ip">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary"
          @click="dialogStatus === 'setVariable' ? setVariable(variabletemp) : getVariable(variabletemp)">确认</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog :title="job_id" :visible.sync="dialogStatusRedoVisible" custom-class="single_dal_view"
      ...
      <div style="width: 100%;background: #fff;padding:0 20px;" class="block">
        <el-steps direction="vertical" :active="init_active">
          <el-step :title="items.title" v-for="(items, key) in statusList" :key="key" :icon="items.icon"
    ...
    <el-dialog :title="job_id" :visible.sync="dialogStatusShowVisible" custom-class="single_dal_view"
      :close-on-click-modal="false" :before-close="beforeDelDestory">
      <div style="width: 100%;background: #fff;padding:0 20px;">
        <el-steps direction="vertical" :active="init_active">
          <el-step :title="init_title" icon="el-icon-more" v-if="init_show"></el-step>
          <el-step :title="computer_title" :status="computer_state" :icon="computer_icon"
            :description="computer_description" v-if="computer_show">
            <template slot="description">
          ...
          <el-step :title="shard_title" :status="storage_state" :icon="shard_icon" :description="shard_description"
            v-if="shard_show">
            <template slot="description">
