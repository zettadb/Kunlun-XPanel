 <template>
  <div>
    ...
    <div v-show="isShowNodeMenuPanel&&g_loading" :style="{left: nodeMenuPanelPosition.x + 'px', top: nodeMenuPanelPosition.y + 'px' }" style="z-index: 999;padding:10px;background-color: #ffffff;border:#eeeeee solid 1px;box-shadow: 0px 0px 8px #cccccc;position: absolute;">
      ...
      <div class="c-node-menu-item" v-if="type==='shard'&& (storage_node_create_priv==='Y')" @click.stop="doAction('重做备机节点')">重做备机节点</div> -->
      <div class="c-node-menu-item" v-if="type==='shard'" @click.stop="doAction('主备切换')">主备切换</div>
...
import {getClusterNodesList,getEffectCluster,getNodes,getAllMachine, getShards,getSnodeTotal,getStandbyNode,getShardPrimary} from '@/api/cluster/list'
import SeeksRelationGraph from 'relation-graph'
import {v4 as uuidv4 } from 'uuid';
import {version_arr,ip_arr,timestamp_arr} from "@/utils/global_variable"
import {getEvStatus,pgEnable,myEnable,delShard,delComp,delSnode,startComp,startSnode,stopComp,stopSnode,restartComp,restartSnode,getClusterDetail,switchShard} from '@/api/cluster/listInterface'
export default {
  name: 'Demo',
    ...
    switchData(row) {
      this.$refs["switchForm"].validate((valid) => {
        if (valid) {
          ...
          let paras={
            shard_id:this.switchtemp.shard_id,
            cluster_id:this.switchtemp.cluster_id,
            master_hostaddr:master_ip,
            assign_hostaddr:assign_ip
          }
