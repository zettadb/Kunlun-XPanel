<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border highlight-current-row
      ...
      <el-table-column v-if="colData[4].istrue" prop="shardtotal" align="center" label="shard分配" width="650">
        <template slot-scope="scope">
      ...
      <el-table-column v-if="colData[5].istrue" prop="back_up" align="center" label="最近备份时间">
        <template slot-scope="scope">
          <span v-if="scope.row.first_backup === ''">无备份</span>
          <span v-else-if="scope.row.first_backup !== ''">{{
            scope.row.first_backup
          }}</span>
        </template>
      </el-table-column>
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="200px">
        ...
        <el-form-item v-if="dialogStatus === 'create'" label="选择计算机:" prop="machinelist">
          <el-tabs type="border-card" v-model="createTypes" id="data-center">
            ...
            <el-tab-pane label="数据中心" name="datacenters">
              <div style="margin:30px">
                <div>
                  购买类型
        ...
        <el-form-item v-show="dialogStatus === 'create' || 'detail'" label="高可用模式:" prop="ha_mode">
          <el-select v-model="temp.ha_mode" placeholder="请选择高可用模式" :disabled="dialogStatus === 'detail'">
            <el-option v-for="item in hamodeData" :key="item.id" :label="item.label" :value="item.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item v-show="dialogStatus === 'detail'" label="shard分配:" prop="shardtotal">
        ...
        <div v-show="isShow" v-if="dialogStatus === 'create' ? true : false">
          ...
          <el-form-item label="每个计算节点的cpu限制模式:" prop="cpu_limit_mode">
            <el-select v-model="temp.cpu_limit_mode" placeholder="请选择">
              <el-option v-for="item in cpuLimitMode" :key="item.id" :label="item.label" :value="item.id" />
            </el-select>
          </el-form-item>
      ...
      <el-form ref="restoreForm" :model="restoretemp" :rules="rules" label-position="left" label-width="110px">
        <el-form-item v-if="dialogStatus === 'restore'" label="选择计算机:" prop="machinelist">
          <el-checkbox-group v-model="restoretemp.machinelist" :min="minMachine" :max="machineTotal">
            <el-checkbox v-for="machine in machines" :key="machine.id" :label="machine.hostaddr">{{ machine.hostaddr }}
            </el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      ...
      <el-form ref="retreatedForm" :model="retreatedtemp" :rules="rules" label-position="left" label-width="120px">
        <el-form-item label="原集群名称:" prop="old_cluster_id">
          ...
          <el-select v-model="retreatedtemp.old_cluster_id" clearable placeholder="请选择原集群名称" style="width: 100%">
            <el-option v-for="item in oldClusterList" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogExpandVisible" custom-class="single_dal_view expand"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="showExpandInfo(expandtemp)">{{
          expandtemp.title
        }}</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogExpandInfoVisible" custom-class="single_dal_view"
      ...
      <el-form ref="expandInfoForm" :model="expandInfoTemp" :rules="rules" label-position="left" label-width="130px">
        ...
        <el-form-item v-if="expandInfoTemp.policy" label="排序方式:" prop="policy_name">
  ...
  methods: {
    ...
    autoChangeShardName(value) {
      ...
      if (this.autoexpandtemp.shard_name) {
        ...
        this.shardsList = []
        const temp = {
          cluster_id: this.autoexpandtemp.list.cluster_id,
          shard_id: this.autoexpandtemp.list.shard_id
        }
          ...
          if (ress.error_code == '0') {
            if (ress.attachment.table_list !== '') {
              ...
              for (let a = 0; a < ss.length; a++) {
                const arr = ss[a].split('.')
                const table_arr = { TABLE_SCHEMA: arr[1], TABLE_NAME: arr[2] }
    ...
    selectionChangeHandle(val) {
      ...
      if (this.expandtemp.shard_name) {
        // 获取shards名称
        const temp = {
          cluster_id: this.expandtemp.list.cluster_id,
          shard_id: this.expandtemp.list.shard_id
        }
        getOtherShards(temp).then((res) => {
          this.shardsList = res.list
        })
    ...
    handleClick(tab) {
      // console.log(tab);
      if (tab.$attrs.value) {
        const ids = tab.$attrs.value.split('_')
      ...
      getStroMachine().then((res) => {
        ...
        if (createCond) {
          ...
          getCompMachine().then((res) => {
            ...
            createCond =
              this.comp_machines === null ||
              this.comp_machines.length === 0 ||
              this.comp_machines === false
            if (createCond) {
              messageTip('请先添加计算机再新增集群!', 'error')
            } else {
    ...
    handleBackUp(row) {
      ...
      //   }
      // });
      handleCofirm('确定要对集群' + row.nick_name + '进行全量备份么?')
        .then(() => {
          const tempData = Object.assign({}, row)
          const backupData = {}
          backupData.user_name = sessionStorage.getItem('login_username')
    ...
    showExpandInfo(row) {
      if (this.expandtemp.title == '自动扩容') {
        ...
        // 查看该集群下有多少个shard
        const temp = { id: row.list.cluster_id }
        getShardsName(temp).then((res) => {
          if (res.list.length < 2) {
            this.message_tips = '集群扩容必须在同一集群不同shard下操作！'
            this.message_type = 'error'
    ...
    expandData(row) {
      ...
      for (let i = 0; i < row.table_list.length; i++) {
        const a =
          row.table_list[i].TABLE_SCHEMA.replace('_$$_', '.') +
          '.' +
          row.table_list[i].TABLE_NAME
        table_list.push(a)
      }
      const tempData = {}
      tempData.user_name = sessionStorage.getItem('login_username')
                ...
                if (this.computer.length > 0) {
                  for (let b = 0; b < ress.attachment.computer_step.length; b++) {
                    if (ress.attachment.computer_step[b].computer_state == 'failed') {
                      ...
                      this.computer_description =
                        ress.attachment.computer_step[b].error_info
                    }
                  }
                }
              } else {
                    ...
                    for (let k = 0; k < this.shard.length; k++) {
                      for (let j = 0; j < ress.attachment.shard_step.length; j++) {
                        if (ress.attachment.shard_step[j].storage_state == 'ongoing') {
                          ...
                          this.shard_description =
                            ress.attachment.shard_step[j].error_info
                        } else if (
                          ress.attachment.shard_step[j].storage_state == 'done'
                        ) {
                          this.shard_active = k
                      ...
                      for (let b = 0; b < ress.attachment.shard_step.length; b++) {
                        if (ress.attachment.shard_step[b].storage_state == 'done') {
                          ...
                          for (let c = 0; c < this.shard.length; c++) {
                            if (
                              this.shard[c].title ==
                              ress.attachment.shard_step[b].shard_hosts
                            ) {
                              this.shard[c].icon = 'el-icon-circle-check'
                              this.shard[c].status = 'success'
                              // current_id=c;
