  ...
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <span
            v-else
            style="color: red;border-bottom: 1px red solid;"
            @click="statusDetail(scope.row)"
      ...
      >
        <template slot-scope="{row}">
          ...
          <el-button
            v-if="storage_node_create_priv==='Y'&&shard_create_priv==='Y'&&compute_node_create_priv==='Y'"
            type="primary"
            size="mini"
    ...
    >
      ...
      >
        ...
        <el-form-item
          v-show="dialogStatus==='create'||'detail'"
          label="每shard中强同步备机应当个数:"
          prop="fullsync_level"
        >
          <el-input
            v-model="temp.fullsync_level"
            class="right_input"
            placeholder="请输入每shard中强同步备机应当个数"
            :disabled="dialogStatus==='detail'"
          >
            <i slot="suffix" style="font-style:normal;margin-right: 10px; line-height: 30px;">个</i>
          </el-input>
        ...
        <div v-show="isShow" v-if="dialogStatus==='create'?true:false">
          ...
          <el-form-item label="每个计算节点的cpu核数:" prop="per_computing_node_cpu_cores">
            <el-input
              v-model="temp.per_computing_node_cpu_cores"
              class="right_input"
              placeholder="请输入每个计算节点的cpu核数"
            />
          </el-form-item>
          ...
          <el-form-item label="每个计算节点最大的存储值:" prop="per_computing_node_max_mem_size">
            <el-input
              v-model="temp.per_computing_node_max_mem_size"
              class="right_input"
              placeholder="请输入每个计算节点最大的存储值"
            >
              <i slot="suffix" style="font-style:normal;margin-right: 10px; line-height: 30px;">MB</i>
            </el-input>
          </el-form-item>
          <el-form-item label="每个存储节点的cpu核数:" prop="per_storage_node_cpu_cores">
            <el-input
              v-model="temp.per_storage_node_cpu_cores"
              class="right_input"
              placeholder="请输入每个存储节点的cpu核数"
            />
          </el-form-item>
          ...
          <el-form-item label="每个存储节点最大存储值:" prop="per_storage_node_max_storage_size">
            <el-input
              v-model="temp.per_storage_node_max_storage_size"
              class="right_input"
              placeholder="请输入每个存储节点最大存储值"
            >
              <i slot="suffix" style="font-style:normal;margin-right: 10px; line-height: 30px;">GB</i>
            </el-input>
          </el-form-item>
        </div>
        <div v-if="dialogStatus==='create'?true:false" :class="isShow === false?'ro-90':'ro90'" @click="toggle"><i
    ...
    >
      ...
      >
        <el-form-item label="原集群名称:" prop="old_cluster_id">
          ...
          <el-select
            v-model="retreatedtemp.old_cluster_id"
            clearable
            placeholder="请选择原集群名称"
            style="width:100%;"
          >
            <el-option
  ...
  methods: {
    ...
    autoChangeShardName(value) {
      ...
      if (this.autoexpandtemp.shard_name) {
        // 获取shards名称
        this.shardsList = []
        const temp = { cluster_id: this.autoexpandtemp.list.cluster_id, shard_id: this.autoexpandtemp.list.shard_id }
    ...
    selectionChangeHandle(val) {
      ...
      if (this.expandtemp.shard_name) {
        // 获取shards名称
        const temp = { cluster_id: this.expandtemp.list.cluster_id, shard_id: this.expandtemp.list.shard_id }
        getOtherShards(temp).then((res) => {
          this.shardsList = res.list
        })
        this.$nextTick(() => {
    ...
    handleCreate() {
      ...
      getStroMachine().then((res) => {
        ...
        let createCond = this.machines === null || this.machines.length === 0 || this.machines === false
        if (createCond) {
          ...
          getCompMachine().then((res) => {
            ...
            createCond = this.comp_machines === null || this.comp_machines.length === 0 || this.comp_machines === false
            if (createCond) {
              messageTip('请先添加计算机再新增集群!', 'error')
            } else {
    ...
    expandData(row) {
      ...
      for (let i = 0; i < row.table_list.length; i++) {
        const a = (row.table_list[i].TABLE_SCHEMA).replace('_$$_', '.') + '.' + row.table_list[i].TABLE_NAME
        table_list.push(a)
      }
      const tempData = {}
      tempData.user_name = sessionStorage.getItem('login_username')
