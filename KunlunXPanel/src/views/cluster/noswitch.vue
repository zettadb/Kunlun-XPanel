      ...
      >
        <el-form-item label="集群名称:" prop="nick_name">
          <div style="max-height: 200px;overflow-y: auto;">
            <el-checkbox v-model="checkAll" :indeterminate="isIndeterminate" @change="handleCheckAllChange">all<span
              style="font-size: 12px;color: #F56C6C;"
            >（勾选all表示所有的集群+所有shard都设置免切）</span></el-checkbox>
            <div style="margin: 15px 0;" />
            <el-checkbox-group v-model="checkedCluster" prop="checkedCluster" @change="handleCheckedCitiesChange">
              <el-checkbox v-for="cluster in clusters" :key="cluster.id" :label="cluster.id">
                {{ cluster.nick_name + "(" + cluster.name + ")" }}
              </el-checkbox>
            </el-checkbox-group>
          </div>
        </el-form-item>
        <el-form-item v-if="checkedCluster.length==1" label="shard名称:" prop="shard_id">
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      const string = '此操作将永久删除集群ID为' + row.cluster_id + '的免切数据,是否继续?code=' + code
