 <template>
  <div>
    ...
    <div v-show="isShowNodeMenuPanel&&g_loading" :style="{left: nodeMenuPanelPosition.x + 'px', top: nodeMenuPanelPosition.y + 'px' }" style="z-index: 999;padding:10px;background-color: #ffffff;border:#eeeeee solid 1px;box-shadow: 0px 0px 8px #cccccc;position: absolute;">
      ...
      <div class="c-node-menu-item"  v-if="(type==='cnode'||type==='snode')&& (user_name=='super_dba')" @click.stop="doAction('禁用')">禁用</div>
      ...
      <div class="c-node-menu-item"  v-else-if="type==='cnode'&& compute_node_drop_priv==='Y'" @click.stop="doAction('删除')">删除</div>
...
//   computed: {
//   point() {
//     if(this.shard_drop_priv==='Y'){
//       this.delpriv=`type==='shard'&& this.shard_drop_priv==='Y'`
//     }else if(this.compute_node_drop_priv==='Y'){
//       this.delpriv=`type==='cnode'&& compute_node_drop_priv==='Y'`;
//     }else if(this.storage_node_drop_priv==='Y'){
//       this.delpriv="type==='snode'&& storage_node_drop_priv==='Y'";
//     }else if(this.shard_drop_priv==='Y'&&this.compute_node_drop_priv==='Y'){
//         this.delpriv="(type==='shard'&& shard_drop_priv==='Y')&&(type==='cnode'&& compute_node_drop_priv==='Y')";
//     }else if(this.shard_drop_priv==='Y'&&this.storage_node_drop_priv==='Y'){
//         this.delpriv="(type==='shard'&& shard_drop_priv==='Y')&&(type==='snode'&& storage_node_drop_priv==='Y')";
//     }else if(this.compute_node_drop_priv==='Y'&&this.storage_node_drop_priv==='Y'){
//          this.delpriv="(type==='cnode'&& compute_node_drop_priv==='Y')&&(type==='snode'&& storage_node_drop_priv==='Y')";
//     }else if(this.shard_drop_priv==='Y'&&this.storage_node_drop_priv==='Y'&& this.compute_node_drop_priv==='Y'){
//         this.delpriv="(type==='shard'&& shard_drop_priv==='Y')&&(type==='cnode'&& compute_node_drop_priv==='Y')&&(type==='snode'&& storage_node_drop_priv==='Y')"
//     }
//     console.log(sessionStorage.getItem('priv'));
//     console.log(this.delpriv);
//     return this.delpriv
//   }
// },
  created(){
        ...
        if(this.currentNode.data.name==='shard'){
          handleCofirm("此操作将永久删除"+this.currentNode.text+", 是否继续?").then( () =>{
            const tempData = {};
              ...
              tempData.timestamp=timestamp_arr[0].time+'';
              const paras={}
              paras.cluster_name=this.currentNode.data.cluster_name;
              paras.shard_name=this.currentNode.text;
              tempData.paras=paras;
          ...
          handleCofirm("此操作将永久删除计算节点"+this.currentNode.text+", 是否继续?").then( () =>{
            const tempData = {};
              ...
              paras.comp_name=this.currentNode.data.comp;
              tempData.paras=paras;
          ...
          handleCofirm("此操作将永久删除存储节点"+this.currentNode.text+", 是否继续?").then( () =>{
            const tempData = {};
              ...
              const paras={};
              paras.cluster_name=this.currentNode.data.cluster_name;
              paras.shard_name=this.currentNode.data.shard_name;
              paras.hostaddr=this.currentNode.data.hostaddr;
              paras.port=this.currentNode.data.port;
              paras.nick_name=this.currentNode.data.nick_name;
              tempData.paras=paras;
